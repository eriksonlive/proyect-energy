export function RoofOrientation(coordinates) {
  //Funcion que permite encontrar la mayor distance entre coordenadas de area dibujada, que probablemente representa la dirección de caída del techo
  ...
  for (let i = 0; i < coordinates.length; i++) {
    ...
    const distance = google.maps.geometry.spherical.computeDistanceBetween(
      new google.maps.LatLng(current_point.lat, current_point.lng),
      new google.maps.LatLng(next_point.lat, next_point.lng)
    );
...
function most_south(bearing1, bearing2) {
  ...
  const diff1 = Math.min(
    (bearing1 - 180 + 360) % 360,
    (180 - bearing1 + 360) % 360
  );
  const diff2 = Math.min(
    (bearing2 - 180 + 360) % 360,
    (180 - bearing2 + 360) % 360
  );
